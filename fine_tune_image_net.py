...
def add_noise(noise):
    def add_noise_to_image(img):
        ...
        return torch.min(
            img + torch.max(torch.randn(img.size()), torch.tensor(0.0)) * std,
            torch.tensor(1.0),
        )
...
def show_examples(ds, seed: int = 1234, examples_per_class: int = 3, size=(350, 350)):
    ...
    grid_original_input = Image.new(
        "RGB", size=(examples_per_class * w, len(labels) * h)
    )

    grid_augmented_input = Image.new(
        "RGB", size=(examples_per_class * w, len(labels) * h)
    )
    ...
    for label_id, _ in enumerate(labels):
        ...
        for i, example in enumerate(ds_slice):
            ...
            augmented, augmented_inputs = TF.to_pil_image(augmented_), TF.to_pil_image(
                augmented_inputs_
            )
...
def load_dataset():
    ...
    train, val, test = datasets.load_dataset(
        "mnist", split=["train", "test[:40%]", "test[40%:100%]"]
    )
...
def main():
    ...
    train, val_with, test_with, test_without = get_prepared_dataset(
        ds, feature_extractor
    )
